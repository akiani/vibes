...
check the reasoning against the data rather than taking it on trust.
...
The loop repeats steps 1–4 until Claude stops asking for queries and writes an
...
The load-bearing detail: **the tool's description is generated from the schema**
(`schema.as_str()`), not hand-written. Add a field to `kg.py` and the agent knows
about it on the next request, with no prompt to keep in sync.
...
small enough to hold in your head while reading the code.
...
answering from general knowledge. That refusal is the behaviour worth testing
when you swap in your own data.
...
**Let the SDK drive the loop.** `agent.py` runs the loop by hand because the loop
is what's being demonstrated. In production you'd likely use
...
The medical data is fictional, simplified, and assembled to make the demo
