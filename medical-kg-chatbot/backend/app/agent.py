...
This is where the grounding happens. Claude gets exactly one tool,
`run_graphql_query`, plus a system prompt forbidding it from answering from
...
`client.beta.messages.tool_runner`, so each step of the loop is visible and so we
can capture each query/result pair for the UI trace.
"""
...
4. Start broad to find IDs, then traverse. If a query returns an error, read the
   message and fix it.
5. Point out connections like these: a patient taking two drugs that interact, or
